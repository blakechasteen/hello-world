...
class DebateStrategy(TemplateStrategy):
    def __init__(self):
    ...
    def description(self) -> str:
        return "Multi-Persona Debate - Conflicting expert viewpoints"
    ...
    def can_apply(self, context: StrategyContext) -> float:
        ...
        if any(keyword in query_lower for keyword in medium_keywords):
            confidence += detection_config['keyword_boost'] * 0.65
    ...
    async def enhance(self, context: StrategyContext) -> StrategyResult:
        ...
        return StrategyResult(
            ...
            metadata={
                'strategy': 'debate',
                'personas': self._full_config['config']['personas'],
                'debate_rounds': self._full_config['config']['debate_rounds']
            },
            confidence=0.91,
            estimated_improvement=0.52
        )
