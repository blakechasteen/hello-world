...
export async function activate(context: vscode.ExtensionContext) {
    ...
    statusBarItem.tooltip = 'Open Squad Agent Panel';
    statusBarItem.show();
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('squad.refactorAndVerify', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor || editor.selection.isEmpty) {
                vscode.window.showWarningMessage('Please select some code first');
                return;
...
async function executeQuery(
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Squad error: ${error}`);
        }
    });
}
...
async function refactorAndVerify(
    ...
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        title: 'Squad: Refactor and Verify',
        cancellable: false
    }, async (progress) => {
            ...
            if (language === 'typescript' || language === 'typescriptreact') {
                ...
                if (diagnostics.length === 0) {
                    ...
                    vscode.window.showInformationMessage(
                        `✅ Refactoring verified! No errors after ${iteration} iteration(s).`
                    );
                } else {
                    ...
                    if (iteration === maxIterations) {
                        vscode.window.showWarningMessage(
                            `⚠️ Could not eliminate all errors after ${maxIterations} iterations. Please review manually.`
                        );
                    }
                }
            } else {
...
async function generateAndRunTests(
    ...
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        title: 'Squad: Generate and Run Tests',
        cancellable: false
    }, async (progress) => {
        ...
        if (language === 'typescript' || language === 'javascript' ||
            ...
            if (failed === 0) {
                ...
                const fix = await vscode.window.showWarningMessage(
                    `⚠️ ${failed} of ${total} tests failed. Would you like Squad to fix them?`,
                    'Fix Tests', 'Show Errors'
                );

                if (fix === 'Fix Tests') {
                    // Iterate to fix failing tests
...
async function createTempFile(code: string, language: string): Promise<vscode.Uri> {
    ...
    const tempPath = `${vscode.workspace.workspaceFolders?.[0]?.uri.fsPath}/.squad_temp.${ext}`;
...
async function createTestFile(code: string, language: string): Promise<string> {
    const ext = language.includes('typescript') ? 'ts' : 'js';
    const testPath = `${vscode.workspace.workspaceFolders?.[0]?.uri.fsPath}/.squad_test.test.${ext}`;
...
async function runTests(testFilePath: string, language: string): Promise<any> {
    ...
    const terminal = vscode.window.createTerminal('Squad Test Runner');
