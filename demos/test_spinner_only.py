...
Uses tmp_path (via tempfile when run standalone) to avoid shared test state (AP-3).
"""
...
async def main(db_dir: Path = None):
    print("\n" + "="*70)
    print("Chat History Spinner Test")
    print("="*70 + "\n")

    # Use provided dir or create a temp directory (AP-3: no hardcoded paths)
    if db_dir is None:
        tmp_ctx = tempfile.TemporaryDirectory()
        db_dir = Path(tmp_ctx.name)
    else:
        tmp_ctx = None

    try:
        test_db = str(db_dir / "spinner_test.db")
