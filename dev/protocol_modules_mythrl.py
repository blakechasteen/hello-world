...
class ComplexityLevel(Enum):
    """Progressive complexity levels for mythRL."""
...
class ProvenceTrace:
    ...
    def add_protocol_call(self, protocol: str, method: str, duration_ms: float, result_summary: str):
    ...
    def add_shuttle_event(self, event_type: str, description: str, data: Optional[Dict] = None):
...
class MythRLResult:
    ...
    def get_performance_summary(self) -> Dict:
        ...
        total_time = time.perf_counter() - self.provenance.start_time
        return {
...
class MemoryBackendProtocol(Protocol):
    ...
    Implementations:
    - InMemoryBackend: Fast, volatile
    - Neo4jBackend: Graph relationships
    - QdrantBackend: Vector similarity
    - HybridBackend: Best of multiple
    """
...
class FeatureExtractionProtocol(Protocol):
    """
    Protocol for feature extraction at multiple scales.
    
    Matryoshka scaling:
    - LITE: 96d embeddings only
    - FAST: 96d + 192d embeddings
    - FULL: 96d + 192d + 384d embeddings  
    - RESEARCH: All scales + experimental features
    """
    
    async def extract_features(self, data: Any, scales: List[int]) -> Dict:
        """Extract features at specified embedding scales."""
        ...
    
    async def extract_motifs(self, data: Any, complexity: ComplexityLevel) -> Dict:
...
class WarpSpaceProtocol(Protocol):
    ...
    NON-NEGOTIABLE: Always present, but complexity-gated
    ...
    async def tension_threads(self, manifold: Dict, threads: List[Dict]) -> Dict:
        """Apply tension to threads in manifold space."""
        ...
    
    async def compute_trajectories(self, manifold: Dict, start_points: List[Dict]) -> Dict:
        """Compute trajectories through manifold (FULL+)."""
        ...
    
    async def experimental_operations(self, manifold: Dict, experiments: List[str]) -> Dict:
...
class ToolExecutionProtocol(Protocol):
    """
    Protocol for tool execution and external integrations.
    
    All complexity levels use this, but routing sophistication varies.
    """
    ...
    async def list_available_tools(self, context: Dict) -> List[Dict]:
        """List tools available in current context."""
        ...
