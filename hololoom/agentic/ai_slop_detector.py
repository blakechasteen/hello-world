...
@dataclass
class SlopIssue:
    """Represents a detected issue in AI-generated code."""
...
class AISlopDetector:
    ...
    Combines hallucination detection with pattern-based detection of
    security issues, bugs, and code quality problems.
    """
    ...
    def __init__(self, indexer=None):
        ...
        Args:
            indexer: Optional CodebaseIndexer for hallucination detection
        """
        self.hallucination_detector = HallucinationDetector(indexer) if indexer else None
    ...
    async def detect_all(
        ...
        if self.hallucination_detector:
            halluc_issues = await self._detect_hallucinations(code, language, file_path)
            issues.extend(halluc_issues)

        # 2. Error handling issues
        issues.extend(self._detect_missing_error_handling(code, language))

        # 3. Hardcoded secrets
        issues.extend(self._detect_hardcoded_secrets(code, language))

        # 4. Resource leaks
        issues.extend(self._detect_resource_leaks(code, language))

        # 5. Security issues
        issues.extend(self._detect_security_issues(code, language))

        # 6. Performance anti-patterns
        issues.extend(self._detect_performance_issues(code, language))

        # 7. Dead code
        issues.extend(self._detect_dead_code(code, language))

        # 8. Inconsistent naming
        issues.extend(self._detect_naming_issues(code, language))

        # 9. Missing documentation
        issues.extend(self._detect_missing_docs(code, language))

        # 10. Incomplete implementation
        issues.extend(self._detect_incomplete_code(code, language))

        # 11. Off-by-one errors
        issues.extend(self._detect_off_by_one(code, language))

        # 12. Timezone issues
        issues.extend(self._detect_timezone_issues(code, language))

        # 13. Copy-paste errors
        issues.extend(self._detect_copy_paste_errors(code, language))
    ...
    async def _detect_hallucinations(
        ...
        hallucinations = await self.hallucination_detector.detect(code, language, file_path)

        for h in hallucinations:
            issues.append(SlopIssue(
                category=SlopCategory.HALLUCINATION,
                severity=SlopSeverity.HIGH,
                line_number=h.line_number,
                column=0,
                description=f"{h.reference} does not exist: {h.reason}",
                context=h.context,
                fix_suggestion=h.fix_suggestion,
