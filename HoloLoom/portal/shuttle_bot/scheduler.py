"""
Simple Scheduler - Picks healthy nodes for job dispatch.

MVP implementation: picks first healthy node.
Future: load balancing, capability matching, affinity.

Methods:
    get_nodes() - Get nodes from Portal
    pick_node() - Pick a healthy node for job dispatch
...
class SimpleScheduler:
    ...
    def __init__(self, portal_url: str, shared_secret: str, timeout: float = 5.0):
        """
    ...
    async def pick_node(self) -> Optional[NodeRecord]:
        """
        Pick a healthy node for job dispatch.
        ...
        # MVP: just pick first node and verify health
        for node in nodes:
            if await self.check_node_health(node):
                logger.info(f"Selected node: {node.node_id}")
                return node
