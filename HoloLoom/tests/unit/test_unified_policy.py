...
def test_mlp_block(runner: TestRunner):
    ...
    def test():
        ...
        print("  Residual connection works ✓")
...
def test_unified_policy_deterministic(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(
            input_dim=128, action_dim=6, policy_type="deterministic", hidden_dims=[256, 256]
        )
...
def test_unified_policy_categorical(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(input_dim=128, action_dim=10, policy_type="categorical")
...
def test_unified_policy_gaussian(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(
            input_dim=128, action_dim=6, policy_type="gaussian", state_dependent_std=True
        )
...
def test_unified_policy_with_icm(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(input_dim=128, action_dim=6, policy_type="gaussian", use_icm=True)
        ...
        print("  Intrinsic reward computed ✓")
...
def test_unified_policy_with_rnd(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(input_dim=128, action_dim=6, policy_type="gaussian", use_rnd=True)
...
def test_unified_policy_hierarchical(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(input_dim=128, action_dim=6, use_hierarchical=True, num_skills=8)
        ...
        print("  Hierarchical outputs correct ✓")

        # Test sampling
        action, info = policy.sample_action(x)
        assert "skill" in info
        print("  Skill in output ✓")
...
def test_ppo_agent_creation(runner: TestRunner):
    ...
    def test():
        ...
        config = PPOConfig(clip_epsilon=0.2, value_loss_coef=0.5, entropy_coef=0.01)
...
def test_ppo_update(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(input_dim=128, action_dim=6, policy_type="gaussian").to(
            runner.device
        )
...
def test_ppo_update_with_curiosity(runner: TestRunner):
    ...
    def test():
        policy = UnifiedPolicy(
            input_dim=128, action_dim=6, policy_type="gaussian", use_icm=True, use_rnd=True
        ).to(runner.device)
        ...
        print("  Curiosity integrated into training ✓")
...
def test_full_pipeline(runner: TestRunner):
    ...
    def test():
        ...
        print("  Rewards computed (extrinsic + intrinsic) ✓")
