...
from HoloLoom.server.agentic_api import app, ServerState
...
def setup_server_state():
    ...
    from HoloLoom.server.agentic_api import state
...
@patch('HoloLoom.server.agentic_api.state')
...
def test_response_matches_typescript_interface(client, mock_orchestrator):
    ...
    """
    with patch('HoloLoom.server.agentic_api.state') as mock_state:
        mock_state.orchestrator = mock_orchestrator
        mock_state.stats = None
